...
let rec fmt_expr fmt expr =
  ...
  | App x -> fprintf fmt "(%a %a)" fmt_expr x.func fmt_expr x.arg
  | Lambda x -> fprintf fmt "(\\ %a -> %a)" fmt_variable x.arg fmt_expr x.body
;;

let rec nodeFmt_expr fmt expr =
  match expr.epre with
  | Var v -> fprintf fmt "(Var %a)" fmt_variable v
  | App x -> fprintf fmt "(App%a,%a)" nodeFmt_expr x.func nodeFmt_expr x.arg
  | Lambda x -> fprintf fmt "(Lambda %a -> %a)" fmt_variable x.arg nodeFmt_expr x.body
;;
...
let fprintf_prog fmt prog = fprintf fmt "%a" fmt_program prog
